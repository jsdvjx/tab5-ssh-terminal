...
static int bsp_get_board_version(void)
{
    ...
    if (i2c_master_probe(bsp_i2c_get_handle(), ESP_LCD_TOUCH_IO_I2C_ST7123_ADDRESS, 100) == ESP_OK) {
        board_ver = 2;
        /* PATCHED (tab5_ssh_term): boards manufactured after 2026-04-28 use the
         * ST7121 panel. Same Sitronix touch at 0x55, but the panel needs a
         * different vendor-unlock key (71 21 vs 71 23) and DPI timing —
         * the ST7123 init is silently ignored and the screen stays black.
         * Discriminate like the factory firmware: touch FW reg 0x0000,
         * 1 => ST7121, 3 => ST7123. */
        esp_lcd_panel_io_handle_t tp_io = NULL;
        esp_lcd_panel_io_i2c_config_t tp_io_cfg = ESP_LCD_TOUCH_IO_I2C_ST7123_CONFIG();
        if (esp_lcd_new_panel_io_i2c(bsp_i2c_get_handle(), &tp_io_cfg, &tp_io) == ESP_OK) {
            uint8_t fw = 0;
            if (esp_lcd_panel_io_rx_param(tp_io, 0x0000, &fw, 1) == ESP_OK && fw == 1) {
                board_ver = 3;
            }
            esp_lcd_panel_io_del(tp_io);
        }
        ESP_LOGI(TAG, "Discovered board version %d (LCD %s, Touch ST71xx)",
                 board_ver, board_ver == 3 ? "ST7121" : "ST7123");
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config, bsp_lcd_handles_t *ret_handles)
{
    ...
    /* PATCHED (tab5_ssh_term): replicate factory bsp_reset_tp() — pulse
     * LCD_RST/TP_RST (expander P4/P5, named *_EN here) low 100ms then high
     * 100ms before panel init. TOUCH feature enable first so the TP_RST pin
     * is configured as an output before we drive it. */
    BSP_ERROR_CHECK_RETURN_ERR(bsp_feature_enable(BSP_FEATURE_TOUCH, true));
    {
        esp_io_expander_handle_t ioex = bsp_io_expander_init();
        esp_io_expander_set_level(ioex, BSP_LCD_EN | BSP_TOUCH_EN, 0);
        vTaskDelay(pdMS_TO_TICKS(100));
        esp_io_expander_set_level(ioex, BSP_LCD_EN | BSP_TOUCH_EN, 1);
        vTaskDelay(pdMS_TO_TICKS(100));
    }
    ...
    /* PATCHED (tab5_ssh_term): ST7121 panel (board v3) — factory DPI vertical
     * timing differs from ST7123 (vsync PW/BP/FP = 20/24/200 vs 2/8/220). */
    ...
    const st7121_vendor_config_t vendor_config_st7121 = {
        .init_cmds      = NULL,   /* driver's built-in ST7121 table: unlock 71 21,
                                   * SLPOUT + 80ms, DISPON + 800ms */
        .init_cmds_size = 0,
        .mipi_config = {
            .dsi_bus = ret_handles->mipi_dsi_bus,
    ...
    } else if (bsp_get_board_version() == 3) {
    ...
    if (bsp_get_board_version() != 3) {
        /* Factory firmware never sends MADCTL (0x36) to the ST7121; its init
         * table relies on the power-on default. Match it. */
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    if (bsp_get_board_version() == 1) {
        ...
        */
        ...
        /* PATCHED (tab5_ssh_term): board v3 (ST7121 panel) shares the same
         * Sitronix touch controller at 0x55. */
