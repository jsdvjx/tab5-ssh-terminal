...
// tab5 port: builder code is excluded by default (saves flash/RAM on target).
// Define IME_PINYIN_BUILD_MODEL (host dictbuilder build) to re-enable it.
#ifdef IME_PINYIN_BUILD_MODEL
#define ___BUILD_MODEL___
#endif

// tab5 port: fixed-width type used for every size field serialized into
// dict_pinyin.dat. Upstream wrote raw size_t (8 bytes on 64-bit hosts,
// 4 bytes on ESP32-P4), making dictionaries non-portable. All fread/fwrite
// of size counters now go through dict_size_t ("v2 uint32" format), so a
// dictionary built on any host loads on the 32-bit target.
typedef uint32_t dict_size_t;
 ...
 */
struct LmaNodeLE0 {
  // tab5 port: were size_t, but the whole struct is fread/fwritten raw into
  // dict_pinyin.dat (dicttrie.cpp). sizeof(LmaNodeLE0) was 24 on the 64-bit
  // dictbuilder host vs 16 on riscv32 ilp32, so the target read garbage
  // nodes and overflowed splid_le0_index_ via bogus spl_idx (heap
  // corruption on P4). uint32 makes the layout 16 bytes everywhere.
  uint32 son_1st_off;
...
// tab5 port: on-disk struct — layout must match between dictbuilder host
// and target.
static_assert(sizeof(LmaNodeLE0) == 16, "LmaNodeLE0 must be 16 bytes");
