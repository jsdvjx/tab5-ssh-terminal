...
SpellingTrie::~SpellingTrie() {
  if (NULL != spelling_buf_)
    ime_free(spelling_buf_);  // tab5 port
...
bool SpellingTrie::construct(const char* spelling_arr, size_t item_size,
  ...
  if (spelling_arr != spelling_buf_) {
    if (NULL != spelling_buf_)
      ime_free(spelling_buf_);  // tab5 port
    spelling_buf_ = static_cast<char*>(
        ime_malloc(item_size * item_num));  // tab5 port
    if (NULL == spelling_buf_)
      return false;
...
bool SpellingTrie::save_spl_trie(FILE *fp) {
  ...
  dict_size_t sz32;
  sz32 = (dict_size_t)spelling_size_;
  if (fwrite(&sz32, sizeof(dict_size_t), 1, fp) != 1)
    return false;

  sz32 = (dict_size_t)spelling_num_;
  if (fwrite(&sz32, sizeof(dict_size_t), 1, fp) != 1)
    return false;
...
bool SpellingTrie::load_spl_trie(FILE *fp) {
  ...
  spelling_buf_ = static_cast<char*>(
      ime_malloc(spelling_size_ * spelling_num_));  // tab5 port
  if (NULL == spelling_buf_)
    return false;
