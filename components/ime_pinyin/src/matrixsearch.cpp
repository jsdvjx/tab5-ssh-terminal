...
bool MatrixSearch::alloc_resource() {
  ...
  share_buf_ = static_cast<size_t*>(ime_malloc(
      (mtrx_nd_size + dmi_size + matrix_size + dep_size) *
      sizeof(size_t)));  // tab5 port: ~?KB pool in PSRAM
...
// tab5 port: initialize from an in-memory dictionary image.
bool MatrixSearch::init_mem(const unsigned char *buf, size_t len,
  ...
  if (!dict_trie_->load_dict_mem(buf, len, 1, kSysDictIdEnd))
    return false;

  if (NULL == fn_usr_dict ||
      !user_dict_->load_dict(fn_usr_dict, kUserDictIdStart, kUserDictIdEnd)) {
    delete user_dict_;
    user_dict_ = NULL;
  } else {
    user_dict_->set_total_lemma_count_of_others(NGram::kSysDictTotalFreq);
...
void MatrixSearch::prepare_candidates() {
  ...
  if (kPrintDebug0) {
    ...
    for (size_t a = 0; a < lpi_total_; a++) {
      printf("[%03u]%d    ", (unsigned)a, lpi_items_[a].psb);  // tab5 port
      if ((a + 1) % 6 == 0) printf("\n");
    }
    printf("\n");
  }
...
char16* MatrixSearch::get_candidate0(char16 *cand_str, size_t max_len,
  ...
  while (mtrx_nd != NULL) {
    ...
    if (kPrintDebug1) {
       printf("---MatrixNode [step: %d, lma_idx: %u, total score:%.5f]\n",
              mtrx_nd->step, (unsigned)mtrx_nd->id, mtrx_nd->score);  // tab5 port
