...
// tab5 port: int32 (matches header decl; int32_t is 'long' on riscv32-newlib)
int32 UserDict::fuzzy_compare_spell_id(
    const uint16 * id1, uint16 len1, const UserDictSearchable *searchable) {
  if (len1 < searchable->splids_len)
    return -1;
  if (len1 > searchable->splids_len)
  ...
  for (i = 0; i < dict_info_.sync_count; i++) {
    ...
    for (j = 0; j < nchar; j++) {
      int ret_len = spl_trie->get_spelling_str16(
          spl[j], ptemp, temp + (sizeof(temp) / sizeof(temp[0])) /* tab5 port: char16 count, not bytes */ - ptemp);
      if (ret_len <= 0)
        break;
      ptemp += ret_len;
      if (ptemp < temp + (sizeof(temp) / sizeof(temp[0])) /* tab5 port: char16 count, not bytes */ - 1) {
        *(ptemp++) = ' ';
      } else {
        j = 0;
        break;
    ...
    int ret_len = utf16le_lltoa(intf, ptemp, temp + (sizeof(temp) / sizeof(temp[0])) /* tab5 port: char16 count, not bytes */ - ptemp);
    if (ret_len <= 0)
      continue;
    ptemp += ret_len;
    if (ptemp < temp + (sizeof(temp) / sizeof(temp[0])) /* tab5 port: char16 count, not bytes */ - 1) {
    ...
    ret_len = utf16le_lltoa(last_mod, ptemp, temp + (sizeof(temp) / sizeof(temp[0])) /* tab5 port: char16 count, not bytes */ - ptemp);
    if (ret_len <= 0)
      continue;
    ptemp += ret_len;
    if (ptemp < temp + (sizeof(temp) / sizeof(temp[0])) /* tab5 port: char16 count, not bytes */ - 1) {
