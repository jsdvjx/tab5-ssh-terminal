...
bool DictBuilder::build_dict(const char *fn_raw,
  ...
  // tab5 port: allocate with ime_malloc so DictTrie::free_resource's
  // ime_free matches (upstream mixed new[] here with free() there).
  dict_trie->root_ = static_cast<LmaNodeLE0*>(
      ime_malloc(lma_nds_used_num_le0_ * sizeof(LmaNodeLE0)));
  dict_trie->nodes_ge1_ = static_cast<LmaNodeGE1*>(
      ime_malloc(lma_nds_used_num_ge1_ * sizeof(LmaNodeGE1)));
  size_t lma_idx_num = homo_idx_num_eq1_ + homo_idx_num_gt1_ + top_lmas_num_;
  dict_trie->lma_idx_buf_ = static_cast<unsigned char*>(
      ime_malloc(lma_idx_num * kLemmaIdSize));
