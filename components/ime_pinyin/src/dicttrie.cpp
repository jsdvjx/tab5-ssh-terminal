...
#include "../include/ime_alloc.h"  // tab5 port: PSRAM-aware allocator
...
void DictTrie::free_resource(bool free_dict_list) {
  // tab5 port: free via ime_free (matches ime_malloc allocations below).
  if (NULL != root_)
    ime_free(root_);
  root_ = NULL;

  if (NULL != splid_le0_index_)
    ime_free(splid_le0_index_);
  splid_le0_index_ = NULL;
  ...
  // tab5 port: upstream freed nodes_ge1_ twice here and leaked lma_idx_buf_.
  if (NULL != lma_idx_buf_)
    ime_free(lma_idx_buf_);
  lma_idx_buf_ = NULL;
...
bool DictTrie::save_dict(FILE *fp) {
  ...
  // tab5 port: serialize size fields as fixed-width dict_size_t (uint32),
  // not raw size_t, so the dictionary is portable between 32/64-bit builds.
  dict_size_t sz;
  sz = (dict_size_t)lma_node_num_le0_;
  if (fwrite(&sz, sizeof(dict_size_t), 1, fp) != 1)
    return false;

  sz = (dict_size_t)lma_node_num_ge1_;
  if (fwrite(&sz, sizeof(dict_size_t), 1, fp) != 1)
    return false;

  sz = (dict_size_t)lma_idx_buf_len_;
  if (fwrite(&sz, sizeof(dict_size_t), 1, fp) != 1)
    return false;

  sz = (dict_size_t)top_lmas_num_;
  if (fwrite(&sz, sizeof(dict_size_t), 1, fp) != 1)
    return false;
...
bool DictTrie::load_dict(FILE *fp) {
  ...
  // tab5 port: size fields are stored as fixed-width dict_size_t (uint32).
  dict_size_t sz;
  if (fread(&sz, sizeof(dict_size_t), 1, fp) != 1)
    return false;
  lma_node_num_le0_ = sz;

  if (fread(&sz, sizeof(dict_size_t), 1, fp) != 1)
    return false;
  lma_node_num_ge1_ = sz;
  ...
  // tab5 port: big dictionary buffers go through ime_malloc (PSRAM on target).
  root_ = static_cast<LmaNodeLE0*>
          (ime_malloc(lma_node_num_le0_ * sizeof(LmaNodeLE0)));
  nodes_ge1_ = static_cast<LmaNodeGE1*>
               (ime_malloc(lma_node_num_ge1_ * sizeof(LmaNodeGE1)));
  lma_idx_buf_ = (unsigned char*)ime_malloc(lma_idx_buf_len_);
  ...
  splid_le0_index_ = static_cast<uint16*>(
      ime_malloc(buf_size * sizeof(uint16)));  // tab5 port
  ...
  // tab5 port: validate spl_idx range before building the quick index — a
  // stale/corrupt dict file must fail the load, not write past
  // splid_le0_index_ (heap smash observed with a layout-mismatched dict).
  for (size_t i = 1; i < lma_node_num_le0_; i++) {
    if (root_[i].spl_idx < kFullSplIdStart ||
        (size_t)(root_[i].spl_idx - kFullSplIdStart) >= buf_size)
      return false;
  }
...
// tab5 port: load from an in-memory dictionary image. fmemopen() exists on
// macOS/Linux hosts and in ESP-IDF newlib, so the FILE*-based loaders can be
// reused unchanged. The caller keeps ownership of buf; it can be freed after
// this returns (everything is copied into heap structures).
bool DictTrie::load_dict_mem(const unsigned char *buf, size_t len,
                             LemmaIdType start_id, LemmaIdType end_id) {
  if (NULL == buf || 0 == len || end_id <= start_id)
    return false;

  FILE *fp = fmemopen(const_cast<unsigned char*>(buf), len, "rb");
  if (NULL == fp)
    return false;
