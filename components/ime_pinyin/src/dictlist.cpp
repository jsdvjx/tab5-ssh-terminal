...
#include "../include/ime_alloc.h"  // tab5 port
...
bool DictList::alloc_resource(size_t buf_size, size_t scis_num) {
  // Allocate memory
  buf_ = static_cast<char16*>(ime_malloc(buf_size * sizeof(char16)));  // tab5 port
  if (NULL == buf_)
    return false;

  scis_num_ = scis_num;

  scis_hz_ = static_cast<char16*>(ime_malloc(scis_num_ * sizeof(char16)));  // tab5 port
  if (NULL == scis_hz_)
    return false;

  scis_splid_ = static_cast<SpellingId*>
      (ime_malloc(scis_num_ * sizeof(SpellingId)));  // tab5 port
...
void DictList::free_resource() {
  // tab5 port: ime_free matches ime_malloc above
  if (NULL != buf_)
    ime_free(buf_);
  buf_ = NULL;
...
bool DictList::save_list(FILE *fp) {
  ...
  // tab5 port: size fields stored as fixed-width dict_size_t (uint32)
  dict_size_t sz32 = (dict_size_t)scis_num_;
  if (fwrite(&sz32, sizeof(dict_size_t), 1, fp) != 1)
    return false;

  dict_size_t arr32[kMaxLemmaSize + 1];
  for (size_t i = 0; i <= kMaxLemmaSize; i++)
    arr32[i] = (dict_size_t)start_pos_[i];
  if (fwrite(arr32, sizeof(dict_size_t), kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;

  for (size_t i = 0; i <= kMaxLemmaSize; i++)
    arr32[i] = (dict_size_t)start_id_[i];
  if (fwrite(arr32, sizeof(dict_size_t), kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;
...
bool DictList::load_list(FILE *fp) {
  ...
  // tab5 port: size fields stored as fixed-width dict_size_t (uint32)
  dict_size_t sz32;
  if (fread(&sz32, sizeof(dict_size_t), 1, fp) != 1)
    return false;
  scis_num_ = sz32;

  dict_size_t arr32[kMaxLemmaSize + 1];
  if (fread(arr32, sizeof(dict_size_t), kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;
  for (size_t i = 0; i <= kMaxLemmaSize; i++)
    start_pos_[i] = arr32[i];
