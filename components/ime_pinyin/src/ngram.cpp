...
NGram::~NGram() {
  // tab5 port: ime_free matches ime_malloc allocations
  if (NULL != lma_freq_idx_)
    ime_free(lma_freq_idx_);

#ifdef ___BUILD_MODEL___
  if (NULL != freq_codes_df_)
    ime_free(freq_codes_df_);
#endif

  if (NULL != freq_codes_)
    ime_free(freq_codes_);
}
...
bool NGram::save_ngram(FILE *fp) {
  ...
  // tab5 port: fixed-width dict_size_t (uint32) in file format
  dict_size_t sz32 = (dict_size_t)idx_num_;
...
bool NGram::load_ngram(FILE *fp) {
  ...
  lma_freq_idx_ = static_cast<CODEBOOK_TYPE*>
                  (ime_malloc(idx_num_ * sizeof(CODEBOOK_TYPE)));
  freq_codes_ = static_cast<LmaScoreType*>
      (ime_malloc(kCodeBookSize * sizeof(LmaScoreType)));
...
bool NGram::build_unigram(LemmaEntry *lemma_arr, size_t lemma_num,
  ...
  if (NULL == freq_codes_df_)
    freq_codes_df_ = static_cast<double*>(
        ime_malloc(kCodeBookSize * sizeof(double)));  // tab5 port
  assert(freq_codes_df_);
  memset(freq_codes_df_, 0, sizeof(double) * kCodeBookSize);

  if (NULL == freq_codes_)
    freq_codes_ = static_cast<LmaScoreType*>(
        ime_malloc(kCodeBookSize * sizeof(LmaScoreType)));  // tab5 port
  assert(freq_codes_);
  ...
  if (NULL == lma_freq_idx_)
    lma_freq_idx_ = static_cast<CODEBOOK_TYPE*>(
        ime_malloc(idx_num_ * sizeof(CODEBOOK_TYPE)));  // tab5 port
